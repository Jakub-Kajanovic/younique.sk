<x-layout-admin>
    <x-breadcrumbs :links="['Admin' => route('dashboard'), 'Všetci autorovia' => route('author.index'), 'Úprava autora' => '#']" class="mb-4" />
    ...
    <form method="POST" action="{{ route('author.update', ['author' => $author->id]) }}" enctype="multipart/form-data">
        @method('PUT')
        @csrf
        <div class="max-w-3xl mx-auto">
            ...
            <div class="my-4">
                ...
                @if($author->photo)
                    <div class="mb-4">
                        <img src="{{ asset('storage/' . $author->photo) }}" alt="Current photo" class="w-32 h-32 object-cover">
                    </div>
                @endif
            
                <input type="file" id="photo" name="photo" class="editor shadow appearance-none border rounded w-full py-2 px-3 text-lightGray leading-tight focus:outline-none focus:shadow-outline"/>
